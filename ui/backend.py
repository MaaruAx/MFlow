...
class Backend(QObject):
    ...
    def _get_spline(self, inp):
        ...
        Point2D inputs (like Center, Pivot) have kf data directly on the input
        and no accessible BezierSpline via GetConnectedOutput — they must be
        written via the input itself. Scalar inputs connect to a BezierSpline tool.
        We try the BezierSpline tool first (correct for handle data), then fall
        back to the input directly.
        """
        # 1. Connected BezierSpline tool — preferred for scalar inputs
        try:
            out = inp.GetConnectedOutput()
            if out:
                tool = out.GetTool()
        ...
        # 2. Input directly — needed for Point2D/compound types (Center, Pivot, etc.)
        try:
